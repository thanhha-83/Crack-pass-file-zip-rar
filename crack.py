...
class Handler:
    ...
    def CheckRules(self):
        ...
        if not self.rules:
            ...
            count = self.maxLength - self.startLength + 1
            if count % 2 == 0 and count % 3 != 0:
                for length in range(self.startLength, self.maxLength + 1, 2):
                    threads = []
                    nThread = Thread(target=self.SendRequest, args=(length, ))
                    nThread.start()
                    threads.append(nThread)
                    nThread = Thread(target=self.SendRequest, args=(length + 1, ))
                    nThread.start()
                    threads.append(nThread)
                    for t in threads:
                        t.join()
            ...
            if count % 3 == 0:
                for length in range(self.startLength, self.maxLength + 1, 3):
                    ...
                    nThread.start()
                    threads.append(nThread)
                    nThread = Thread(target=self.SendRequest, args=(length + 2, ))
                    nThread.start()
                    threads.append(nThread)
                    for t in threads:
                        t.join()
